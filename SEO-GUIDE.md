...
- New `Speakable` schema for Google Assistant / voice search — flags the title and meta description (via `xpath`) as the "say-aloud" content. These live in the static HTML `<head>`, so the markup validates without depending on JS-rendered DOM elements. The `WebPage` node's `isPartOf` references the `WebSite` node (`#website`), the valid target type for that property.
