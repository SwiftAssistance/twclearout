...
// --- GLOBAL DATA (Defined globally for maximum performance and scoping stability) ---
...
// --- STABLE SUB-COMPONENTS (Defined outside App to prevent re-renders and scroll-to-top issues) ---

const ReviewCard = ({ review, idx }) => (
  <div className="w-full flex-shrink-0 px-2 h-full">
    <div className={`p-5 md:p-14 border-8 border-slate-900 rounded-[2rem] md:rounded-[3rem] shadow-[10px_10px_0px_#ecf3ef] md:shadow-[20px_20px_0px_#ecf3ef] flex flex-col relative overflow-hidden transition-all ${review.color} h-full min-h-[280px] md:min-h-[460px]`}>
      ...
      <div className="relative z-10 flex-grow text-left mb-4 md:mb-10">
        ...
        {/* Adjusted font sizes for mobile to ensure the card never overflows */}
        <p className="text-sm md:text-3xl lg:text-4xl font-[1000] uppercase italic leading-tight md:leading-[1.2] tracking-tight text-balance">
          "{review.text}"
        </p>
      </div>

      <div className="flex items-center gap-3 md:gap-4 pt-4 border-t border-current border-opacity-10 text-left">
        <div className={`w-10 md:w-16 h-10 md:h-16 rounded-full flex items-center justify-center font-[1000] border-2 md:border-4 border-slate-900 text-xs md:text-xl italic shrink-0 ${idx % 2 === 0 ? 'bg-[#16a34a] text-white' : 'bg-white text-[#16a34a]'}`}>
          {review.initials}
        </div>
        <div className="overflow-hidden">
          <p className="font-[1000] uppercase text-[10px] md:text-lg leading-none truncate">{review.name}</p>
          <p className="font-bold opacity-60 text-[8px] md:text-xs uppercase italic truncate mt-0.5">{review.location} • Verified Review</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <header className="relative min-h-[85vh] md:min-h-screen flex items-center pt-24 overflow-hidden bg-[#064e3b]">
    <div className="absolute inset-0 z-0">
      <img 
        ...
        alt="Industrial waste logistics" 
    ...
    <div className="absolute inset-0 opacity-[0.15] pointer-events-none" aria-hidden="true">
      <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <defs><pattern id="industrial-grid-v4" width="80" height="80" patternUnits="userSpaceOnUse"><path d="M 80 0 L 0 0 0 80" fill="none" stroke="#22c55e" strokeWidth="0.5"/></pattern></defs>
        <rect width="100%" height="100%" fill="url(#industrial-grid-v4)" />
      </svg>
    </div>
    ...
    <div className="container mx-auto px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16">
        <div className="lg:col-span-7 bg-[#dcfce7] p-10 md:p-20 flex flex-col justify-end min-h-[400px] border-4 border-slate-900 shadow-[12px_12px_0px_#16a34a] relative overflow-hidden group">
          ...
          <p className="font-bold text-lg md:text-xl text-slate-700 max-w-sm italic leading-relaxed text-balance">Industrial warehouses and multi-floor office blocks. Professional logistical removal.</p>
        </div>
        ...
        <a href="services.html" className="lg:col-span-4 bg-slate-900 p-10 border-4 border-slate-900 flex flex-col justify-between text-white group hover:bg-orange-500 transition-all duration-500 cursor-pointer">
        ...
        <article className="lg:col-span-8 bg-[#ecf3ef] border-4 border-slate-900 p-8 md:p-12 flex flex-col md:flex-row items-center gap-10 shadow-lg">
          ...
          <div className="md:w-1/2 overflow-hidden rounded-lg border-2 border-slate-900 h-64 w-full order-1 md:order-2">
            {/* FIXED IMAGE URL FOR EXTERIOR RECOVERY */}
            <img src="https://images.unsplash.com/photo-1591336395884-633009a05531?auto=format&fit=crop&q=80" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" alt="Professional garden clearance" loading="lazy" />
          </div>
        </article>
      </div>
    </div>
          ...
          <div className="bg-slate-50 p-6 md:p-10 border-4 border-slate-900 rounded-lg text-left">
             <form className="space-y-8 text-left" onSubmit={e => e.preventDefault()}>
               <div className="grid md:grid-cols-2 gap-8 text-left">
                  ...
                  <div className="text-left">
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Postcode Area</label>
                    <input type="text" placeholder="e.g. RG1" className="w-full bg-white border-4 border-slate-900 p-4 md:p-5 font-black uppercase text-xs outline-none focus:border-[#16a34a]" />
                  </div>
               </div>
               <div className="text-left">
                  <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Phone Number</label>
                  <input type="tel" placeholder="07xxx xxxxxx" className="w-full bg-white border-4 border-slate-900 p-4 md:p-5 font-black uppercase text-xs outline-none focus:border-[#16a34a]" />
               </div>
...
const App = () => {
  ...
  // Dedicated Reviews Rendering with proper scope
      ...
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-12 gap-12 md:gap-20 items-start">
          ...
          <div className="lg:col-span-8 relative">
            {/* THIS DIV NOW HAS overflow-hidden TO MASK PREVIOUS CARDS */}
            <div className="relative overflow-hidden touch-pan-y" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              <div className="flex flex-nowrap transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]" style={{ transform: `translateX(-${currentReview * 100}%)` }}>
                {reviewsData.map((review, idx) => (
                  <ReviewCard key={`review-${idx}`} review={review} idx={idx} />
                ))}
              </div>
            </div>
