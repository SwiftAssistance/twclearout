...
const App = () => {
  ...
  // Brand Color Constant for consistency (Lighter Green)
  const brandGreen = "#4ade80"; 
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-orange-500 selection:text-white overflow-x-hidden">
          ...
          <div className={`hidden xl:flex items-center gap-8 font-black text-[11px] uppercase tracking-[0.2em] transition-colors duration-300 ${isScrolled ? 'text-slate-600' : 'text-slate-200'}`}>
            {navLinks.map(link => (
              <a 
                key={link.id} 
                href={link.href} 
                className={`transition-all relative group py-2 ${activeSection === link.id ? 'text-[#4ade80]' : 'hover:text-[#4ade80]'}`}
              >
                {link.name}
            ...
            <a 
              ...
              className="bg-[#4ade80] hover:bg-slate-900 hover:text-white text-black px-6 py-3 rounded-sm flex items-center gap-3 transition-all shadow-md font-black italic uppercase tracking-wider"
            >
            ...
            <div className="flex flex-col gap-6 font-black text-xl md:text-2xl uppercase tracking-widest italic text-slate-900 mb-12">
              {navLinks.map(link => (
                <a 
                  ...
                  className={`border-b border-slate-100 pb-4 flex justify-between items-center transition-colors ${activeSection === link.id ? 'text-[#4ade80]' : 'active:text-[#4ade80]'}`}
                >
                  {link.name} <ChevronRight size={24} className={activeSection === link.id ? 'text-[#4ade80]' : 'text-slate-200'} />
                </a>
              ))}
            </div>

            <div className="mt-auto space-y-6 text-center">
              <a href="tel:08001234567" className="bg-[#4ade80] text-black w-full p-6 text-center rounded-sm flex items-center justify-center gap-4 font-black italic text-xl uppercase shadow-lg">
      ...
      {/* --- HERO SECTION --- */}
      <header id="home" className="relative min-h-screen flex items-center pt-24 overflow-hidden bg-[#04241a]">
        <div className="absolute inset-0 opacity-[0.15] pointer-events-none">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="green-grid" width="60" height="60" patternUnits="userSpaceOnUse">
                <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#4ade80" strokeWidth="1"/>
              </pattern>
            </defs>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-8">
              <div className="inline-flex items-center gap-3 bg-[#0a3a2a] border border-[#4ade80]/40 px-5 py-2.5 rounded-sm text-[10px] md:text-[11px] font-black text-[#4ade80] mb-8 tracking-[.25em] uppercase italic">
              ...
              <div className="flex flex-wrap gap-6 items-center">
                <a 
                  href="#quote" 
                  className="bg-orange-500 hover:bg-orange-400 text-black px-12 py-6 rounded-sm font-black text-xl uppercase italic tracking-wider flex items-center gap-4 transition-all hover:-translate-y-1 shadow-[8px_8px_0px_rgba(74,222,128,1)]"
                >
      ...
      {/* --- STATS BAR --- */}
      <section className="bg-[#4ade80] py-8 border-y-4 border-black relative z-20">
        <div className="container mx-auto px-6 text-black font-black">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-10">
            {stats.map((s, idx) => (
              <div key={idx} className="flex items-center gap-4 md:gap-5">
                <div className="bg-black text-[#4ade80] p-2 md:p-3 rounded-sm shrink-0">{s.icon}</div>
                <div className="flex flex-col leading-none">
                  <span className="text-2xl md:text-3xl uppercase italic leading-none">{s.value}</span>
                  <span className="text-[9px] md:text-[10px] uppercase tracking-widest opacity-90">{s.label}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-slate-900">
            ...
            <article className="lg:col-span-3 bg-white group overflow-hidden relative border-4 border-black h-[400px] md:h-[500px] cursor-pointer shadow-lg">
              ...
              <div className="relative z-20 p-8 h-full flex flex-col justify-between text-white">
                <div className="bg-orange-500 text-black w-12 md:w-14 h-12 md:h-14 flex items-center justify-center font-black border-2 border-black text-xl italic">01</div>
                <div>
      ...
      <section id="compliance" className="py-32 bg-white">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-20">
            <div>
              <h2 className="text-[#4ade80] font-black uppercase tracking-[0.4em] text-xs mb-6 italic underline">Audit & Duty of Care</h2>
      ...
      {/* --- FULL ENHANCED FOOTER --- */}
      <footer className="bg-[#04241a] pt-24 pb-12 border-t-8 border-[#4ade80]">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-4 gap-12 md:gap-20 mb-20 text-white">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <p className="text-slate-400 max-w-sm font-bold italic text-lg leading-relaxed underline decoration-[#166534]">
                Berkshire & Surrey's premier disposal service. 94% recycling rate and 100% legal compliance on every job.
              </p>
              <div className="flex flex-wrap gap-4">
                 <div className="bg-[#0a3a2a] border-2 border-white/5 px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-300 italic flex items-center gap-2">
                   <ShieldCheck size={14} className="text-[#4ade80]" /> Registered Carrier
                 </div>
                 <div className="bg-[#0a3a2a] border-2 border-white/5 px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-300 italic flex items-center gap-2">
                   <Scale size={14} className="text-[#4ade80]" /> Insured £5M PLI
                 </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <h5 className="font-black text-[#4ade80] uppercase tracking-[0.3em] text-xs italic">Clearance Services</h5>
              <ul className="space-y-4 font-black text-xs uppercase tracking-widest italic text-slate-400">
                {['House Clearances', 'Builders Waste', 'Garden Removal', 'Office Rip-outs', 'Sofa Disposal', 'Shed Demolition'].map(service => (
                  <li key={service} className="hover:text-white hover:translate-x-2 transition-all cursor-pointer">
                    {service}
                  </li>
                ))}
              </ul>
            ...
            <div className="space-y-8 flex flex-col items-start lg:items-end text-left lg:text-right">
               <div className="space-y-2">
                 ...
                 <a href="tel:08001234567" className="text-4xl md:text-5xl font-[1000] text-white hover:text-orange-500 transition-colors italic tracking-tighter leading-none block">
                   0800 123 4567
                 </a>
               </div>
          ...
          <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
            <p className="text-slate-600 text-[9px] font-black uppercase tracking-[0.4em] text-center md:text-left leading-relaxed">
            ...
            <div className="flex gap-6 items-center">
              {['Privacy', 'Legal', 'Duty of Care'].map(item => (
                <a key={item} href="#" className="text-[10px] font-black uppercase text-slate-500 hover:text-white transition-colors italic tracking-widest">{item}</a>
              ))}
