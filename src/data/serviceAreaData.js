...
const SERVICE_AREA_DATA = {
  "house-clearance": {
    ...
    getTagline: (area) =>
      `Professional house clearout & clearance in ${area.name}, ${area.county}. Same-day service, fixed pricing, fully licensed and insured.`,
    getIntro: (area) =>
      `Need a house clearout in ${area.name}? Total Waste Clearout specialises in fast, compassionate house clearouts and full property clearances across ${area.name} and the wider ${area.county} area. Whether you call it a house clearout, house clearance, or property clear-up — our experienced crews handle everything from single rooms to complete whole-house clearouts across all ${area.postcodes[0]} postcodes.`,
    getDescription: (area) =>
    ...
    getFaqs: (area) => [
      { q: `Do you do house clearouts in ${area.name}?`, a: `Yes — whether you call it a house clearout or house clearance, we do exactly the same job. We clear the entire property in ${area.name} including all furniture, belongings, white goods, and rubbish. Same-day house clearouts are available across all ${area.postcodes[0]} postcodes.` },
      { q: `How quickly can you do a house clearout in ${area.name}?`, a: `We offer same-day house clearouts across all ${area.name} postcodes including ${area.postcodes.join(", ")}. Most standard 2-3 bedroom house clearouts are completed within a single day. For larger properties, we can arrange multi-day clearances.` },
      { q: `How much does a house clearout in ${area.name} cost?`, a: `Single room clearouts in ${area.name} start from £150, with full 2-3 bed house clearouts from £400. We provide fixed-price quotes with no hidden fees — the price we quote is the price you pay. Every quote includes loading, transport, and disposal.` },
      { q: `Do you handle probate house clearances in ${area.name}?`, a: `Yes — probate clearances and house clearouts are one of our specialities in ${area.name}. We work sensitively with families and can provide full inventory documentation for solicitors. We set aside any items of value or sentimental importance before clearing.` },
      { q: `Do you clear the entire house including garage and shed?`, a: `Absolutely. Our ${area.name} house clearout service covers every part of the property — all rooms, garage, shed, garden, and any outbuildings. We leave the property completely empty and broom-clean.` },
      { q: `Are you licensed for house clearouts in ${area.name}?`, a: `Yes — we're fully registered waste carriers with the Environment Agency, carrying £5M public liability insurance. We provide legal waste transfer notes for every house clearout in ${area.name} and the wider ${area.county} area.` }
    ],
    getMeta: (area) => ({
      title: `House Clearout & Clearance ${area.name} | ${area.postcode} | Same-Day Service`,
      description: `House clearout & clearance in ${area.name}, ${area.county}. Probate, downsizing & landlord clearouts. Same-day service, 94% recycled, licensed carrier. From £150. Call 07769 844298.`,
      keywords: `house clearout ${area.name.toLowerCase()}, house clearance ${area.name.toLowerCase()}, home clearout ${area.name.toLowerCase()}, home clearance ${area.name.toLowerCase()}, probate clearance ${area.name.toLowerCase()}, property clearout ${area.name.toLowerCase()}, property clearance ${area.name.toLowerCase()}, furniture removal ${area.name.toLowerCase()}, ${area.postcode} house clearout, ${area.postcode} house clearance, ${area.county.toLowerCase()} house clearout, ${area.county.toLowerCase()} house clearance, bereavement clearance ${area.name.toLowerCase()}, full house clearout ${area.name.toLowerCase()}, estate clearance ${area.name.toLowerCase()}, deceased estate clearance ${area.name.toLowerCase()}, downsizing clearance ${area.name.toLowerCase()}, landlord house clearout ${area.name.toLowerCase()}, cheap house clearout ${area.name.toLowerCase()}, same day house clearout ${area.name.toLowerCase()}, house clearout near me ${area.name.toLowerCase()}, hoarder clearance ${area.name.toLowerCase()}, whole house clearout ${area.name.toLowerCase()}, sofa removal ${area.name.toLowerCase()}, mattress removal ${area.name.toLowerCase()}, white goods removal ${area.name.toLowerCase()}, appliance removal ${area.name.toLowerCase()}`
    })
  },
  ...
  "end-of-tenancy-clearance": {
    ...
    getMeta: (area) => ({
      ...
      keywords: `end of tenancy clearance ${area.name.toLowerCase()}, tenant clearance ${area.name.toLowerCase()}, rental clearance ${area.name.toLowerCase()}, move out clearance ${area.name.toLowerCase()}, landlord clearance ${area.name.toLowerCase()}, flat clearance ${area.name.toLowerCase()}, ${area.postcode} end of tenancy, ${area.county.toLowerCase()} tenancy clearance, deposit return clearance ${area.name.toLowerCase()}, tenant clearout ${area.name.toLowerCase()}, rental property clearance ${area.name.toLowerCase()}, end of lease clearance ${area.name.toLowerCase()}, letting agent clearance ${area.name.toLowerCase()}, student house clearance ${area.name.toLowerCase()}, HMO clearance ${area.name.toLowerCase()}, same day tenancy clearance ${area.name.toLowerCase()}, cheap end of tenancy clearance ${area.name.toLowerCase()}, emergency tenant clearance ${area.name.toLowerCase()}, tenant rubbish removal ${area.name.toLowerCase()}, flat clearout ${area.name.toLowerCase()}`
    })
  },
  ...
  "garden-waste-removal": {
    ...
    getMeta: (area) => ({
      ...
      keywords: `garden waste removal ${area.name.toLowerCase()}, green waste clearance ${area.name.toLowerCase()}, garden clearance ${area.name.toLowerCase()}, tree removal ${area.name.toLowerCase()}, hedge clearance ${area.name.toLowerCase()}, garden rubbish ${area.name.toLowerCase()}, ${area.postcode} garden waste, ${area.county.toLowerCase()} garden clearance, soil removal ${area.name.toLowerCase()}, garden skip alternative ${area.name.toLowerCase()}, tree branch removal ${area.name.toLowerCase()}, hedge trimming waste ${area.name.toLowerCase()}, grass cutting waste ${area.name.toLowerCase()}, turf removal ${area.name.toLowerCase()}, garden furniture disposal ${area.name.toLowerCase()}, fencing removal ${area.name.toLowerCase()}, decking removal ${area.name.toLowerCase()}, landscaping waste ${area.name.toLowerCase()}, same day garden waste collection ${area.name.toLowerCase()}, cheap garden waste removal ${area.name.toLowerCase()}, compost waste collection ${area.name.toLowerCase()}, green bin alternative ${area.name.toLowerCase()}`
    })
  },
  ...
  "construction-waste-removal": {
    ...
    getMeta: (area) => ({
      ...
      keywords: `construction waste removal ${area.name.toLowerCase()}, skip alternative ${area.name.toLowerCase()}, building waste ${area.name.toLowerCase()}, rubble removal ${area.name.toLowerCase()}, builder waste ${area.name.toLowerCase()}, trade waste collection ${area.name.toLowerCase()}, ${area.postcode} construction waste, ${area.county.toLowerCase()} building waste, renovation waste ${area.name.toLowerCase()}, skip hire alternative ${area.name.toLowerCase()}, rubble clearance ${area.name.toLowerCase()}, plasterboard disposal ${area.name.toLowerCase()}, timber waste removal ${area.name.toLowerCase()}, concrete removal ${area.name.toLowerCase()}, demolition waste ${area.name.toLowerCase()}, kitchen refit waste ${area.name.toLowerCase()}, bathroom refit waste ${area.name.toLowerCase()}, builder skip alternative ${area.name.toLowerCase()}, cheap skip alternative ${area.name.toLowerCase()}, same day builder waste ${area.name.toLowerCase()}, no permit waste removal ${area.name.toLowerCase()}, construction site clearance ${area.name.toLowerCase()}`
    })
  },
  ...
  "garage-shed-clearance": {
    ...
    getMeta: (area) => ({
      ...
      keywords: `garage clearance ${area.name.toLowerCase()}, shed clearance ${area.name.toLowerCase()}, garage demolition ${area.name.toLowerCase()}, shed removal ${area.name.toLowerCase()}, outbuilding clearance ${area.name.toLowerCase()}, garage clearout ${area.name.toLowerCase()}, ${area.postcode} garage clearance, ${area.county.toLowerCase()} shed demolition, shed demolition ${area.name.toLowerCase()}, garden building removal ${area.name.toLowerCase()}, concrete garage removal ${area.name.toLowerCase()}, prefab garage demolition ${area.name.toLowerCase()}, workshop demolition ${area.name.toLowerCase()}, summerhouse removal ${area.name.toLowerCase()}, shed dismantling ${area.name.toLowerCase()}, garage contents clearance ${area.name.toLowerCase()}, cheap garage clearance ${area.name.toLowerCase()}, same day shed removal ${area.name.toLowerCase()}, outbuilding demolition ${area.name.toLowerCase()}, garden shed clearance ${area.name.toLowerCase()}, asbestos garage removal ${area.name.toLowerCase()}`
    })
  }
};
